...
<body style="">
<form name="BankPayRequestForm" id="BankPayRequestForm" method="post" action="http://10.10.10.113:8094/onePayService/onePay.do">
   <input type="hidden" 	name="merchant_id"  value="${param.merchant_id}" />
   <input type="hidden"   	name="xml_enc"  value="${param.xml_enc}" />
   <input type="hidden"  	name="key_enc"  value="${param.key_enc}" />
   <input type="hidden"  	name="sign"  value="${param.sign}" />
   <input type="submit" id="new_submit" value="提交到银行" style="display:none" /> 
</form>
...
window.onload=function(){
	document.getElementById("BankPayRequestForm").submit();
};/**/
</script>
</body>
</html>